...
| Variable               | Required           | Default                       | Description                                                                 | Example                           | Security   |
| ---------------------- | ------------------ | ----------------------------- | --------------------------------------------------------------------------- | --------------------------------- | ---------- |
| `COMGATE_MERCHANT_ID`  | Yes                | -                             | Comgate merchant identifier from portal.comgate.cz                          | `123456`                          | Public     |
| `COMGATE_SECRET`       | Yes (for payments) | -                             | Comgate merchant secret key for webhook verification and API authentication | `your-comgate-secret-from-portal` | **Secret** |
| `COMGATE_API_URL`      | No                 | `https://payments.comgate.cz` | Comgate API base URL                                                        | `https://payments.comgate.cz`     | Public     |
| `COMPANY_DEFAULT_IBAN` | No                 | -                             | Default IBAN for QR payment generation (when company IBAN not set)          | `CZ6508000000192000145399`        | Public     |

**Note:** Test mode is controlled by NODE_ENV. When NODE_ENV=production, payments use real cards. No separate test mode flag exists.

---

## Payment Cron

Environment variables for automated payment expiration via external cron service (Railway Cron / cron-job.org).

| Variable                  | Required       | Default | Description                                                                            | Example                   | Security   |
| ------------------------- | -------------- | ------- | -------------------------------------------------------------------------------------- | ------------------------- | ---------- |
| `CRON_SECRET`             | Yes (for cron) | -       | Bearer token for cron endpoint authentication (`/api/v1/payments/expire-pending/cron`) | `openssl rand -base64 32` | **Secret** |
| `PAYMENT_TIMEOUT_MINUTES` | No             | `30`    | Minutes before a pending payment expires and the booking is cancelled                  | `30`, `60`                | Public     |
...
# Payments (NODE_ENV=development uses Comgate sandbox automatically)
COMGATE_MERCHANT_ID=test-merchant
COMGATE_SECRET=test-secret-key
...
# Payments (production mode — real cards when NODE_ENV=production)
COMGATE_MERCHANT_ID=123456
COMGATE_SECRET=production-comgate-secret-from-portal
CRON_SECRET=production-cron-secret-min-32-chars
