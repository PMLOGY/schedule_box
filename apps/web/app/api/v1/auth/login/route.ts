...
export async function POST(req: NextRequest) {
  ...
  return trace.getTracer('schedulebox').startActiveSpan('schedulebox.auth.login', async (span) => {
    try {
      ...
      if (userRecord.mfaEnabled) {
        if (!input.mfa_code) {
          ...
          span.setAttributes({ 'auth.mfa_required': true });
          span.end();
          logRouteComplete({
      ...
      if (userRecord.companyId && userRecord.roleName !== 'admin') {
        ...
        if (companyRecord?.suspendedAt) {
          span.setAttributes({ 'auth.company_suspended': true });
          span.end();
          logRouteComplete({
            route: '/api/v1/auth/login',
            method: 'POST',
      ...
      span.setAttributes({ 'auth.login_success': true });
      span.end();
      logRouteComplete({
        route: '/api/v1/auth/login',
        method: 'POST',
